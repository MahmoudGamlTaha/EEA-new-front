...
export class OperationsLayoutComponent implements OnInit {
  ...
  constructor(private operationsService: OperationsService , private routeService: RouteService) {
    this.sideBarItems = this.operationsService.customerSidebarItems;
  }
  ngOnInit() {
    this.dataSubscription = this.routeService.currentRouteData$.subscribe(data => {
