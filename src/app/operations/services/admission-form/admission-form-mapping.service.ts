...
export class AdmissionFormMappingService {
  ...
  uploadAttchment(file, status, key) {
    ...
    this.countSteps+=1
    console.log('this.countSteps' , this.countSteps)
    if(file instanceof  File) {
    let formData = new FormData();
    formData.append('files', file, file.name);
    return this.utilitiesApiService.uploadFile(formData).subscribe((res) => {
      this.counter += 1;
      this.uploadResponseMapping(res, status, key, this.counter);
  ...
  getAttachmentByKey(key) {
    return this.customerRequestDataFromApi['requestDetail'].find(detail => detail['companyAcceptance'].fileField == key)
  }

  uploadResponseMapping(res, status, key, count) {
    if (status === 'companyAcceptance') {
  ...
  getOtherAttachmentObjFromRequest(attachments) {
    let attachment  =  attachments.find(attachment => attachment.fileField === 'any')
    return [{
      id: attachment?.id,
  ...
  getDocumentArrFromRequest(document) {
    ...
    if(document) this.attachmentsArr.push({
      id: document.id,
      url: document.url,
