...
export class StatisticsComponent {
  ...
  totals;
  chartCardsArr;
  constructor(private statisticsService: StatisticsService, private requestSubmitted:RequestSubmittedService) {}
  ...
  initNotification(){
    this.requestSubmitted.getCustomerRequests().subscribe(response => {
      // this.importBoxService.initNotifications(response['content']);
        this.statisticsService.setNotification(response['content']);
