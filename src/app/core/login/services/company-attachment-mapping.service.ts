...
export class CompanyAttachmentMappingService {
  ...
  getAttachmentsList() {
    ...
    const attachmentsFormsKeys = Object.keys(this.formData).filter((key) =>
      typeof this.formData[key]  === "object" 
    );
    const attachments = {};
    // map attachments with Objects of each Attachment Field ex.{File, From , To}
    attachmentsFormsKeys.forEach((key) => {attachments[key] = this.formData[key];});
  ...
  } // else to display missing files warning 
}
