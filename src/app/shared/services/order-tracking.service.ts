...
export class OrderTrackingService {
  ...
  initOrderTrackingList(trackingListResponse , isRdf) {
    ...
    for(let i =  0 ; i< this.orderTrackingList.length ; i++) {
      ...
      let apiItem = trackingListResponse.find(item => item.administrativeId === orderItem.administrativeId)
      if(apiItem) {
        orderItem.status = apiItem.status;
        orderItem.state = apiItem.state 
        orderItem.date = apiItem.createdDate ? this.utilsService.convertDateWithTime(apiItem.createdDate) : null
      }
    }
  ...
  getOrderTrackingByRequestId(id){
    const url = this.baseUrl + '/portal/customer-request/track-request-status?requestId='+id;
